...
class GameScene: SKScene {
    ...
    override func keyDown(with event: NSEvent) {
        ...
        case 0x7B: //left
            let moveAction = SKAction.moveBy(x: -20, y: 0, duration: 1)
        ...
        case 0x7D: //down
            let moveAction = SKAction.moveBy(x: 0, y: -20, duration: 1)
            let myNode = childNode(withName: "fly")
            let myBombNode = childNode(withName: "bombe")
            myNode?.run(moveAction)
            if fired==false {
        ...
        case 0x7E: //up
            let moveAction = SKAction.moveBy(x: 0, y: 20, duration: 0.1)
            let myNode = childNode(withName: "fly")
            let myBombNode = childNode(withName: "bombe")
            myNode?.run(moveAction)
            if fired==false {
